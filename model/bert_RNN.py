...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        output:
            output: [batch_size, seq_length, num_directions * lstm_hidden]
            h_n: [num_layers * num_directions, batch_size, lstm_hidden]
            c_n: [num_layers * num_directions, batch_size, lstm_hidden]
        """
        self.lstm = nn.LSTM(config.hidden_size, config.lstm_hidden, config.num_layers,
                            bidirectional=True, batch_first=True, dropout=config.dropout)

        self.dropout = nn.Dropout(config.dropout)

        self.fc_rnn = nn.Linear(config.lstm_hidden * config.num_layers, config.num_classes)
    ...
    def forward(self, x):
        ...
        # out: [batch_size, seq_length, num_directions * lstm_hidden]
